...
namespace ChannelEngine.Api.Client.Model
{
    ...
    public partial class EntitiesAddressModels :  IEquatable<EntitiesAddressModels>, IValidatableObject
    {
        ...
        { 
            ...
            {
                yield return new ValidationResult("Invalid value for HouseNr, length must be less than 50.", new [] { "HouseNr" });
            }

            // HouseNr (string) minLength
            if(this.HouseNr != null && this.HouseNr.Length < 0)
            {
                yield return new ValidationResult("Invalid value for HouseNr, length must be greater than 0.", new [] { "HouseNr" });
            }

            // HouseNrAddition (string) maxLength
            if(this.HouseNrAddition != null && this.HouseNrAddition.Length > 50)
            {
                yield return new ValidationResult("Invalid value for HouseNrAddition, length must be less than 50.", new [] { "HouseNrAddition" });
            }

            // HouseNrAddition (string) minLength
            if(this.HouseNrAddition != null && this.HouseNrAddition.Length < 0)
            {
                yield return new ValidationResult("Invalid value for HouseNrAddition, length must be greater than 0.", new [] { "HouseNrAddition" });
            }

            // Region (string) maxLength
            if(this.Region != null && this.Region.Length > 50)
            {
                yield return new ValidationResult("Invalid value for Region, length must be less than 50.", new [] { "Region" });
            }

            // Region (string) minLength
            if(this.Region != null && this.Region.Length < 0)
            {
                yield return new ValidationResult("Invalid value for Region, length must be greater than 0.", new [] { "Region" });
            }

            // CountryIso (string) maxLength
            if(this.CountryIso != null && this.CountryIso.Length > 2)
            {
                yield return new ValidationResult("Invalid value for CountryIso, length must be less than 2.", new [] { "CountryIso" });
            }

            // CountryIso (string) minLength
            if(this.CountryIso != null && this.CountryIso.Length < 0)
            {
                yield return new ValidationResult("Invalid value for CountryIso, length must be greater than 0.", new [] { "CountryIso" });
            }

            // Original (string) maxLength
            if(this.Original != null && this.Original.Length > 256)
            {
                yield return new ValidationResult("Invalid value for Original, length must be less than 256.", new [] { "Original" });
            }

            // Original (string) minLength
            if(this.Original != null && this.Original.Length < 0)
            {
                yield return new ValidationResult("Invalid value for Original, length must be greater than 0.", new [] { "Original" });
            }
